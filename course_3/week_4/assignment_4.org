...
So all the addresses are in detroit (which is what you'd expect, but I thought I'd double-check). Assuming these addresses work, there's no reason to keep the other addresses, I would imagine.

Since the address has the location street number, and name we can probably drop it.
...
For some reason, when I didn't put ~regex=True~ in there it missed some of the zip codes (but not all of them). Now lets try the merge.
...
def clean_x(source):
    ...
    """
    drops = ["compliance", "violator_name"]
    ...
    cleaned = cleaned.dropna(sipbest=)
